...
public class KdTree
{
    ...
    public TreeInstance this[int key]
    {
        get
        {
            if (key >= _count)
    ...
    public void Add(TreeInstance item)
    {
    ...
    public void AddAll(List<TreeInstance> items)
    {
    ...
    /// Remove at position i (position in list or loop)
    /// </summary>
    public void Remove(KdNode item)
    {
        var list = new List<KdNode>(_getNodes());
        list.Remove(item);
        Clear();
    ...
    public void RemoveAll(Predicate<TreeInstance> match)
    {
        var list = new List<KdNode>(_getNodes());
    ...
    public int CountAll(Predicate<TreeInstance> match)
    {
        int count = 0;
    ...
    public IEnumerator<TreeInstance> GetEnumerator()
    {
        var current = _root;
        while (current != null)
        {
    ...
    public List<TreeInstance> ToList()
    {
    ...
    public TreeInstance FindClosest(Vector3 position)
    {
    ...
    public TreeInstance[] FindClose(Vector3 position)
    {
        var output = new List<TreeInstance>();
        _findClosest(position, output);
        return output.ToArray();
    }


    protected TreeInstance _findClosest(Vector3 position, List<TreeInstance> traversed = null)
    {
        if (_root == null) Debug.LogError("nevalja");
    ...
    protected List<KdNode> FindWithinRadius(Vector3 position, float radius)
    {
        List<KdNode> withinRadius = new List<KdNode>();
        if (_root == null) Debug.LogError("Root node is null.");
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            if (splitSearch < splitCurrent)
            {
                if (current.left != null)
                    _open[openAdd++] = current.left; //go left
                if (Mathf.Abs(splitCurrent - splitSearch) * Mathf.Abs(splitCurrent - splitSearch) < radius && current.right != null)
                    _open[openAdd++] = current.right; //go right
            }
            else
            {
                if (current.right != null)
                    _open[openAdd++] = current.right; //go right
                if (Mathf.Abs(splitCurrent - splitSearch) * Mathf.Abs(splitCurrent - splitSearch) < radius && current.left != null)
                    _open[openAdd++] = current.left; //go left
            }
        }
    ...
    public void RemoveWithinRadius(Vector3 position, float radius)
    {
        List<KdNode> withinRadius = FindWithinRadius(position, radius);
        foreach (var treeInstance in withinRadius)
        {
            Remove(treeInstance);
        }
    }
...
public class KdNode
{
    internal TreeInstance component;
    internal int level;
