...
public class TerraiFlatten : EditorBehaviour
{
    ...
    public override void EditorUpdate(EditorManager editor)
    {
        ...
        if (timer > time)
        {
            ...
            if (Input.GetMouseButton(0) && editing)
            {
                ...
                {
                    FlattenTerrain(hit.textureCoord * editor.terrain.terrainData.heightmapResolution, editor.terrain, editor);
    ...
    void FlattenTerrain(Vector2 pos, Terrain terrain, EditorManager editor)
    {
    ...
    private float[,] DrawBrushStatic(float x, float y, float[,] pixels, int resolution, float radious, float multiplyer)
    {
        ...
        for (int i = (int)(x - (radious / 2)); i < (int)(x + (radious / 2)); i++)
        {
            for (int j = (int)(y - (radious / 2)); j < (int)(y + (radious / 2)); j++)
            {
                ...
                if (i >= 0 && j >= 0 && j < resolution && i < resolution)
                {
                    //pixels[j, i] += Input.GetKey(KeyCode.LeftShift) ? -1 : 1 * 0.0001f * multiplyer * (
                    // manager.BrushImage.GetPixelBilinear((float)i / radious, (float)j / radious).r);
