...
public class AIType : ScriptableObject
{
    ...
    {
        List<UnitController> enemyAsNeighbour = new List<UnitController>();
        foreach (UnitController controller in player.Towers)
        {

            foreach (UnitController neighbour in controller.neighbours)
            {
        ...
        if (enemyAsNeighbour.Count > 0)
        {
            ...
            foreach (UnitController neighbour in enemyAsNeighbour)
            {
